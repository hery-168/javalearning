 ...
 * @Description ConcurrentLinkedDeque 案例
 * 双向链表结构的无界并发队列
 * 与ConcurrentLinkedQueue  的区别是该阻塞队列同时支持FIFO和FILO两种操作方式，即可以从队列的头和尾同时操作(插入/删除)
 * 适合“多生产，多消费”的场景
 * 自旋+CAS的非阻塞算法来保证线程并发访问时的数据一致性
 *
 * 需要注意的点：
 * 1.size方法需要遍历链表
 * 2.ConcurrentLinkedDeque的迭代器是弱一致性的，在一个线程遍历队列结点而另一个线程尝试对某个队列结点进行修改的话不会抛出异常
